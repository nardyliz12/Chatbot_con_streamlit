...
def cargar_distritos():
    try:
        distritos = pd.read_csv('distritos.csv')['Distrito'].tolist()
        st.sidebar.write("Distritos de reparto cargados:", len(distritos))
        return distritos
    except FileNotFoundError:
        st.error("No se pudo encontrar el archivo de distritos de reparto. Verifica que 'distritos_reparto.csv' exista.")
        return []
...
if prompt:
    ...
    if "messages" not in st.session_state:
        ...
        st.session_state.messages.append({"role": "assistant", "content": "¡Bienv
